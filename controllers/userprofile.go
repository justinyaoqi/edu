...
// oprations for Userprofile
type UserprofileController struct {
	beego.Controller
}

func (this *UserprofileController) URLMapping() {
...
// @Success 200 {int} models.Userprofile.Id
...
func (this *UserprofileController) Post() {
	...
	if id, err := models.AddUserprofile(&v); err == nil {
		data.Error = 0
...
// @router /:id [get]
func (this *UserprofileController) GetOne() {
...
// @Failure 403
// @router / [get]
func (this *UserprofileController) GetAll() {
	var fields []string
	var sortby []string
	var order []string
	...
	if v := this.GetString("query"); v != "" {
		for _, cond := range strings.Split(v, ",") {
			...
			if len(kv) != 2 {
				data.Error = 1
				data.Result = errors.New("Error: invalid query key/value pair")
				this.Data["json"] = data
				this.ServeJson()
				return
			}
...
// @router /:id [put]
func (this *UserprofileController) Put() {
	idStr := this.Ctx.Input.Params[":id"]
	id, _ := strconv.Atoi(idStr)
...
// @Failure 403 id is empty
// @router /:id [delete]
func (this *UserprofileController) Delete() {
	idStr := this.Ctx.Input.Params[":id"]
	id, _ := strconv.Atoi(idStr)
	var data models.Data
